...
@solution
	...
	集合的top-father的定义就是这个集合中最前面的点。
	每个点开始时自己单独为一个集合，father就是自己。
	每个点对应一个val值，这个值的含义是father点到本点的奇偶性。初始化为0。

	对于这个题目，只是统计区间的奇偶个数，如果两个区间没有overlap，是无法推出矛盾的，
	如果两个区间有overlap，但是没有前后对齐，也是无法推出矛盾的，
	例如1-2之间有奇数个，2-3之间有奇数个，
	那么当arr[2]==1时，arr[1]==arr[3]==0满足条件
	当arr[2]==0时，arr[1]==arr[3]==1满足条件

	所以要想推出矛盾，必需在一个集合中推出矛盾，
	也就是说，在某一次合并的时候，top-father一致（这相当于左端对齐了），
	但是推出奇偶矛盾。
